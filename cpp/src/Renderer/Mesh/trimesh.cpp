			...
			{
				{
					...
					for (int v = 0; v < textureHeight; v++)
					{
						...
						{
							h_textureMap[3 * v * textureWidth + 3 * u + 0] = 255.f;
							h_textureMap[3 * v * textureWidth + 3 * u + 1] = 0.f;
							h_textureMap[3 * v * textureWidth + 3 * u + 2] = 0.f;
						}
					}
...
void trimesh::setupGPUMemory()
{
	...
	if (fh.fail())
	{
		std::cerr << "loadSkinningData: File not found." << std::endl;
	}
	else
	{
		// some state variables
...
void trimesh::updateGPUVertexColorToRGB()
{
	for (unsigned int i = 0; i < N; i++)
	{
		...
		char channel1UChar = (char)channel1Int;
		char channel2UChar = (char)channel2Int;
		char channel3UChar = (char)channel3Int;
